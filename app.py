...
if compute_button:
    # 1. Compute peak current amplitude I0 = V0 / |Z|
    omega = 2 * np.pi * f
    ...
    # 5. Create a dummy dipole
    dipole = misc.Dipole(
        moment=(0.0, 0.0, num_turns * A_tx),
        position=(0.0, 0.0, 0.0),
    )

    # 6. Loop: update dipole and compute B
    ...
    ax2.plot(t, B_vals)
    ax2.set_title("Magnetic Dipole Field $|\\mathbf{B}(t)|$ at $(z_0,0,0)$")
    ax2.set_xlabel("Time (s)")
    ax2.set_ylabel("$|\\mathbf{B}(t)|$ [T]")
    ax2.grid(True)

    st.pyplot(fig)

    # 8. Display computed peak current
